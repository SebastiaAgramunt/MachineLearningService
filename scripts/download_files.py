...
def _data_preparation(config: ParseDataPreprocessing):
    ...
    numerical = [elem for elem in df._get_numeric_data().columns if
                 elem not in ["class"]]

    categorical = [feature for feature in list(df.columns) if
                   feature not in numerical+["class"]]
    ...
    model = RandomForestClassifier(n_estimators=100, criterion='gini',
                                   max_depth=None)
    selector = RFE(model, config.max_features, step=config.step_del_features,
                   verbose=0)
    selector = selector.fit(X_train.values, y_train.values)

    selected_features = X_train.loc[:, selector.support_].columns.\
        values.tolist()
    print("\nThe {} variables that predict the best are: \n{}".
          format(config.max_features, ',\n'.join(selected_features)))

    X_train.loc[:, selector.support_].to_csv(config.data_dir+'/processed/'
                                             + 'X_train.csv', index=False)
    X_test.loc[:, selector.support_].to_csv(config.data_dir+'/processed/'
                                            + 'X_test.csv', index=False)
    y_train.to_csv(config.data_dir+'/processed/'+'y_train.csv', index=False,
                   header=True)
    y_test.to_csv(config.data_dir+'/processed/'+'y_test.csv', index=False,
                  header=True)
...
def _data_normalisation(config: ParseDataPreprocessing):
    ...
    print("fp: {} values predicted as positive but actually were negative".
          format(fp))
    print("fn: {} values predicted as negative but actually were positive".
          format(fn))
    ...
    X_train.to_csv(config.data_dir+'/processed/'+'X_train_norm.csv',
                   index=False, header=False)
    X_test.to_csv(config.data_dir+'/processed/'+'X_test_norm.csv',
                  index=False, header=False)

    y_train.to_csv(config.data_dir+'/processed/'+'y_train_norm.csv',
                   index=False, header=False)
    y_test.to_csv(config.data_dir+'/processed/'+'y_test_norm.csv',
                  index=False, header=False)
